...
test('production ingress preserves proxy identity and stays bound to localhost', () => {
  ...
  const publicPortCompose = readFileSync(join(projectRoot, 'docker-compose.public-port.yml'), 'utf8')
  ...
  assert.match(containerNginx, /proxy_set_header X-Real-IP \$remote_addr/)
  assert.match(containerNginx, /proxy_set_header X-Forwarded-Proto \$scheme/)
  assert.match(containerNginx, /limit_req zone=skyfleet_api/)
  assert.match(compose, /127\.0\.0\.1:\$\{DEMO_HTTP_PORT:-8088\}:8080/)
  assert.match(publicPortCompose, /0\.0\.0\.0:\$\{DEMO_HTTP_PORT:-8088\}:8080/)
